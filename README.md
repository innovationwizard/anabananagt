...
- **El import map del panel depende del entorno**: `src/app/(payload)/admin/importMap.js` registra los componentes cliente que el panel necesita, y su contenido depende de qué plugins están activos al generarlo. Con las variables `S3_*` presentes (producción), el plugin de almacenamiento inyecta un componente (`S3ClientUploadHandler`) que DEBE estar en el mapa — si falta, `/admin` se renderiza **en blanco, sin ningún error en consola**. Por eso el build de producción (`npm run ci`) regenera el mapa con el entorno real (`payload generate:importmap`) antes de compilar. Si agregas componentes admin o plugins, ejecuta `npm run generate:importmap` y commitea el resultado.
- Para verificar el panel en un build local (`npm run build && npm run start`), ábrelo **en un navegador** — el HTML no basta: el panel siempre se renderiza en el cliente.
